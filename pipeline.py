...
class Pipeline(object):
    ...
    def __init__(self, modifiers, filters, calculations, publishers):
        ...
        :param filters: the filters that the contours which was detected in the frame will pass trough
        :type filters: list<IFilter>
        :param calculations: the calculations that will be enabled on the contours
        :type calculations: list<ICalculation>
        :param publishers: different ways to publish the results of the calculations
    ...
    def process_image(self, frame):
        ...
        # f represents a filter (called f to avoid naming it like the built in function filter)
        for f in self.filters:
            contours = f.filter(contours)

        contour = contours[0]  # represents the first contour that passed all of the filters
        # this iteration responsible for publishing via different publishers the results of the calculations
